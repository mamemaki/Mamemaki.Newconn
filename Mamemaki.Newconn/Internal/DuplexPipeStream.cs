...
public class DuplexPipeStream(IConnectionStateFeature connection, 
    PipeReader input, PipeWriter output, bool throwOnCancelled = false) : Stream
{
    [Conditional("VERBOSE")]
...
#pragma warning restore CA2012
    ...
    {
        return WriteAsync([], 0, 0, cancellationToken);
    }
    ...
    private async ValueTask<int> ReadAsyncInternal(Memory<byte> destination, CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (true)
            {
                ...
                DebugLog($"data arrived {result.Buffer.Length} bytes ({result.IsCompleted}, {result.IsCanceled})");
                var readableBuffer = result.Buffer;
                try
                {
                    if (throwOnCancelled && result.IsCanceled && _cancelCalled)
                    {
                        DebugLog("exit: canceled");
                        // Reset the bool
                        _cancelCalled = false;
                    ...
                    if (!readableBuffer.IsEmpty)
                    {
                        ...
                        DebugLog("read completed");
                        return count;
                    }

                    if (result.IsCompleted)
                    {
                        DebugLog("pipe completed");
                        return 0;
                    }
                }
                finally
        ...
        catch (Exception ex)
        {
            if (!connection.IsClosed)
            {
                DebugLog($"exit: unexpected error({ex.Message}, {ex?.InnerException?.Message})");
                throw;
            }
            DebugLog($"exit: expected error({ex.Message}, {ex?.InnerException?.Message})");
            return 0;
        }
    }
